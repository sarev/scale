...
| `scale_javascript.py` | JS worker — tree-sitter (ESM + CommonJS) + `iter_block_targets_js` block provider (nested-function-aware structural segmenter) + `cognitive_complexity_js` (the native escalation routing score) |
| `scale_c.py` | C worker — tree-sitter + `iter_block_targets_c` block provider (structural segmenter) + `cognitive_complexity_c` (the native escalation routing score) + the header/implementation **doc-site** layer (`iter_decls_with_info_c` prototype collection, `plan_doc_sites_c`/`CDocPlan`/`CFileDocPlan` the model-free `--doc-site` planner; the def pass skips redirected definitions and documents prototypes from the impl body) |
...
| `scale_escalate.py` | Selective escalation to a stronger model: `Escalation` policy/collector (per-routine merged requests), codestats-JSON loader, `routine_text_hash` (the C span-hash identity), `run_manifest` (the run-level version-2 manifest with cross-request `snippet_ref` slimming), `unfilled_answers` (the completeness counter), manifest read/write (v1 read-upgraded) |
