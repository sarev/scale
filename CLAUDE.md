...
| `scale_python.py` | Python worker — CPython `ast` based (definition pass + `iter_block_targets` block provider + `structural_segments` deterministic block segmenter + `cognitive_complexity`/`node_sig` for escalation + `elide_structurally` for snippet elision) |
...
- **Segment pass** — produces the chunk `(start, end)` ranges. **Python uses a deterministic structural segmenter** (`scale_python.structural_segments`), *not* the model: a provider precomputes the chunks and sets `BlockTarget.segments`, and `annotate_blocks` uses them when present (falling back to the LLM segment pass below only for languages/targets with no structural segmenter). The structural rules place a paragraph break — only ever at a legal boundary line — for: the first statement after a docstring (when the body has one); the statement after a nested def/class (a def clearly ends a paragraph, ungated); a `return` whose preceding statement is at the same indent; a compound/nested-def block of ≥ `_SEG_MIN_BLOCK_LINES` (3) source lines; and the statement resuming after such a block closes (a dedent). **Size, not cognitive complexity, gates triviality** (cc is nesting-dominated and wrongly demotes long-but-flat blocks). This is free, reproducible, and — being model-free — also makes the escalation manifest's chunk recipe deterministic across emit/apply. It was developed/validated against a corpus with `tests/block_eval/segment_eval.py` (model-free harness: wall a file, re-segment, diff vs the human paragraphing). **LLM fallback** (`request_segments`, used for JS/C): the body is rendered as a numbered view (`render_numbered_body`) with a line number **only on boundary lines** (over-long non-boundary runs collapse to a `« N lines elided »` band carrying no number); the model is asked for **chunk line ranges** (`start-end`), *not* split points — framing it as "group related lines" stops a weak model fragmenting the body line-by-line. `_parse_segments` snaps each start to a legal boundary (drops illegal starts) and clamps ends within the body / before the next chunk. Run **deterministically** (`SEGMENT_TEMPERATURE = 0.0`).
